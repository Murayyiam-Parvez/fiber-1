...
We will briefly explain fiber's workflow here with the examples under *examples* folder. Basically, we prepared some security patches under *examples/patches* folder, our reference kernel *examples/refkernel* adopts all these patches but *examples/nopatchkernel* does not. We then generate binary signatures (stored in *examples/sigs*) for these patches and then use them to test the patch presence for the target kernel (*examples/targetkernel*,  Samsung S7 kernel released in 2016/09/09). The test result can be found in *examples/targetkernel/matchresults* where **P** means the related patch has been adopted and **N** otherwise.
...
For example,examples/refkernel
...
The "vmlinux" (generated by the compiler) for the reference kernel. We need this because it contains fine-grained DWARF debug information that can help to map source code lines to binary instructions.
- *tmp_o*: debug info
The debug info file extracted from vmlinux
(Due to size limit we haven't included the vmlinux/tmp_o file for reference kernel in example/refkernel folder, you can download it from here https://drive.google.com/drive/folders/1XNXqL0aMKK6Po4vO2mHo6keL91knw3Qr?usp=sharing)
...
**output**:
we will store signatures ext_list dir/sigs. Thus we don't need to specify the output path
...
- *target kernel path*
the kernel  that needs to be tested. (eg. *examples/imgs/targetkernel*)
**NOTE**:
